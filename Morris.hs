...
minimax (Node (m,pos,del,p) l) n b  
              | b = maximum [minimax child (n-1) False | child<-l]
...
heuristic m =   5  * (countMils m PlayerB) 
              - 20 * (countMils m PlayerA) 
              + 2  * (countCloseMils m PlayerB) 
              - 3  * (countCloseMils m PlayerA)
